...
Public Class Fmail
    ...
    Private Sub Fmail_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If FnovoProtocolo.Visible = True Then
            cbEmail.SelectedIndex = FnovoProtocolo.cbDestinatario.SelectedIndex
            If FnovoProtocolo.cbOrigem.SelectedIndex = 0 Then
                tbAssunto.Text = "Protocolo " & FnovoProtocolo.tbProtocolo.Text & " da Ouvidoria"
            Else
                tbAssunto.Text = "Protocolo " & FnovoProtocolo.tbProtocolo.Text & " da Ouvidoria - OuvidorSUS"
            End If

        ElseIf FeditarProtocolo.Visible = True Then
            cbEmail.SelectedIndex = FeditarProtocolo.cbEditarDestinatario.SelectedIndex
            If FeditarProtocolo.cbEditarOrigem.SelectedIndex = 0 Then
                tbAssunto.Text = "Protocolo " & FeditarProtocolo.tbEditarProtocolo.Text & " da Ouvidoria"
            Else
                tbAssunto.Text = "Protocolo " & FeditarProtocolo.tbEditarProtocolo.Text & " da Ouvidoria - OuvidorSUS"
            End If
    ...
    Private Sub btSendEmail_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btSendEmail.Click
        ...
        If m.getDataset("SELECT 1contato FROM ouvidoria WHERE id =" & FmainOuvidoria.dgListProtocolos.CurrentRow.Cells(1).Value).Rows(0).Item(0).ToString = "" Then
            m.SQLupdate("ouvidoria", "1contato='" & m.mysqlDateFormat(Date.Today) & "'", "id", FmainOuvidoria.dgListProtocolos, FmainOuvidoria.dgListProtocolos.CurrentRow.Cells(1).Value, Nothing, False, False, "", False, 1)
            FmainOuvidoria.loadProtocolos("WHERE ouvidoria.sistema = '" & FmainOuvidoria.cbBuscaOrigem.Text & "' AND ouvidoria.`status` = 'Em andamento'")
        Else
            m.SQLupdate("ouvidoria", "2contato='" & m.mysqlDateFormat(Date.Today) & "'", "id", FmainOuvidoria.dgListProtocolos, FmainOuvidoria.dgListProtocolos.CurrentRow.Cells(1).Value, Nothing, False, False, "", False, 1)
            FmainOuvidoria.loadProtocolos("WHERE ouvidoria.sistema = '" & FmainOuvidoria.cbBuscaOrigem.Text & "' AND ouvidoria.`status` = 'Em andamento'")
        End If
